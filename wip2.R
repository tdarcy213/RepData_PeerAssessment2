...
phealthSmry<-ggplot(healthSmry[order(EV_YEAR)],aes(x=EV_YEAR,y=SUM_HEALTH,fill=EVTYPE))
phealthSmry+geom_bar(color="black",stat="identity",position=position_dodge())+ggtitle(expression(atop("NOAA STORM DATA", atop(italic("1950 - 2011"), ""))))+xlab("YEAR")+ylab("HEALTH")+scale_fill_brewer(palette="Paired")+geom_text(data = maxhealthSmry, aes(label=SUM_HEALTH),hjust=1.1, vjust=0,size=3,color="blue4")
peconSmry<-ggplot(econSmry[order(EV_YEAR)],aes(x=EV_YEAR,y=SUM_ECON,fill=EVTYPE))
peconSmry+geom_bar(color="black",stat="identity",position=position_dodge())+ggtitle(expression(atop("NOAA STORM DATA", atop(italic("1950 - 2011"), ""))))+xlab("YEAR")+ylab("TOTAL ECONOMIC DAMAGE")+scale_fill_brewer(palette="Paired")+geom_text(data = maxeconSmry, aes(label=SUM_ECON),hjust=1.1, vjust=0,size=3,color="blue4")
...
ptop5health<-ggplot(top5health[order(TOT_HEALTH)],aes(x=EVTYPE,y=TOT_HEALTH,fill=EVTYPE))
ptop5health+geom_bar(color="black",stat="identity",position=position_dodge())+ggtitle(expression(atop("NOAA STORM DATA", atop(italic("1950 - 2011"), ""))))+xlab("EVENT")+ylab("HEALTH")+scale_fill_brewer(palette="Paired")+geom_text(data = maxhealthTot, aes(label=TOT_HEALTH),hjust=2, vjust=0,size=4,color="blue4")+theme(axis.text.x = element_blank())
ptop5econ<-ggplot(top5econ[order(TOT_ECON)],aes(x=EVTYPE,y=TOT_ECON,fill=EVTYPE))
ptop5econ+geom_bar(color="black",stat="identity",position=position_dodge())+ggtitle(expression(atop("NOAA STORM DATA", atop(italic("1950 - 2011"), ""))))+xlab("EVENT")+ylab("ECON")+scale_fill_brewer(palette="Paired")+geom_text(data = maxeconTot, aes(label=TOT_ECON),hjust=1.5, vjust=0,size=4,color="blue4")+theme(axis.text.x = element_blank())
